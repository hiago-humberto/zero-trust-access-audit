...
def aplicar_regras_zero_trust(df_desligados, df_acessos):
    # Projeção Explícita: Garante que só vamos usar essas colunas e ignora o resto
    df_desligados = df_desligados[['Matrícula', 'Nome', 'Cargo', 'Área', 'Data Desligamento']].copy()
    ...
    condicoes_risco = [
        ...
        (df_cruzamento['Status'].str.upper() == 'ATIVO'),
    ...
    # Agora é 100% seguro extrair a Área
...
def aplicar_regras_transferencias(df_transferidos, df_acessos):
    # Projeção Explícita
    df_transferidos = df_transferidos[['Matrícula', 'Nome', 'Cargo Origem', 'Área Origem', 'Data Transferência', 'Cargo Destino', 'Área Destino']].copy()
