...
if modo_auditoria == "1. Revogação de Acessos (Desligados)":
    ...
    if arquivo_base and arquivo_usuarios:
        ...
        # Filtro atualizado para o novo padrão do motor
        df_falhas = df_resultado[df_resultado['Classificação de Risco'] != '🟢 OK']
...
# ==========================================
    ...
    arquivo_ativos = st.sidebar.file_uploader("Base de Ativos (TXT)", type=['txt']) 
    arquivo_usuarios = st.sidebar.file_uploader("Acessos do Sistema (TXT)", type=['txt'])
    ...
    if arquivo_desligados and arquivo_transferidos and arquivo_ativos and arquivo_usuarios:
        ...
        # Construção segura da lista para o gráfico (evita colisão e erro de DataFrame vazio)
        lista_falhas = []
        if not df_falhas_desl.empty:
            lista_falhas.append(df_falhas_desl[['Area_Risco', 'Classificação de Risco']])
        if not df_falhas_transf.empty:
            lista_falhas.append(df_falhas_transf[['Area_Risco', 'Classificação de Risco']])
        if not df_falhas_fantasmas.empty:
            lista_falhas.append(df_falhas_fantasmas[['Area_Risco', 'Classificação de Risco']])
            
        if lista_falhas:
            df_todas_falhas = pd.concat(lista_falhas)
            ...
            st.success("🎉 Nenhum risco detectado nas áreas analisadas. Controle Efetivo!")
